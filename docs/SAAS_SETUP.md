...
- `AUTH_EMAIL_ENDPOINT`: endpoint HTTPS del proveedor/adaptador que enviará los correos.
- `AUTH_FROM_EMAIL`: remitente verificado, por ejemplo `Planorha <cuentas@dominio.com>`.

### Secretos

Estos valores deben cargarse como secretos de Cloudflare y nunca confirmarse en el repositorio:

- `AUTH_EMAIL_TOKEN`: token del adaptador de correo, cuando corresponda.
...
## Contrato del adaptador de correo

Planorha realiza un `POST` a `AUTH_EMAIL_ENDPOINT` con el siguiente JSON:
...
Cuando existe `AUTH_EMAIL_TOKEN`, se envía como `Authorization: Bearer <token>`.
...
2. Configurar correo y Turnstile en Preview.
